...
const getBaseUrl = () => {
    ...
    if (urlParams.has('api_base_url')) {
        localStorage.setItem('api_base_url', urlParams.get('api_base_url') as string)
        return urlParams.get('api_base_url') as string
    } else if (localStorage.getItem('api_base_url')) {
        return localStorage.getItem('api_base_url') as string
